...
function RenderItem(props) {
  ...
  }/image?url=${encodeURIComponent(photo.thumbnailLink)}`;
          ...
          if (resp) {
            resp.json().then((data) => {
              if (Date.now() - data.timestamp < 5 * 60 * 1000) {
                ...
                fetch(
                  `https://keyol.vercel.app/images?folderId=${
                    import.meta.env.VITE_FOLDER_ID
                  }&folderPath=${this.props.location.pathname.substring(1)}`,
                )
                  .then((response) => response.json())
                  .then((res) => {
                    const data = new Response(
                      JSON.stringify({ folders: res, timestamp: Date.now() }),
                    );
            ...
            fetch(
              `${import.meta.env.VITE_BASE_API}/images?folderId=${
                import.meta.env.VITE_FOLDER_ID
              }&folderPath=${this.props.location.pathname.substring(1)}`,
            )
              .then((response) => response.json())
  ...
  render() {
    ...
    return (
      <>
        <Modal
          {...modal}
          close={() => this.setState({ modal: { state: false, src: "" } })}
        />
        <Content>
          ...
          {folders?.folders?.map(({ files, name }) => (
            <PhotoAlbum
              layout="rows"
              photos={files}
